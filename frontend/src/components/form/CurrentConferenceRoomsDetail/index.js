...
export default class my extends Component {
    ...
    getListData = (value) => {
        ...
        Object.keys(this.state.calenderListData||{}).forEach(mth=>{
            ...
            if(mth==month){
                Object.keys(this.state.calenderListData[mth]||{}).forEach(innerDay=>{
                    console.log('day',innerDay);
                    if(day==innerDay){
                        console.log('this.state.calenderListData[month][day]',this.state.calenderListData[month][day]);
    ...
    disabledDate = (value) => {
        return !(value.date()>=new Date().getDate()&&value.date()<new Date().getDate()+7);
    }
    selectHandle = (val)=>{
        console.log('selectHandle',val._d.getMonth(),val._d.getDate());
        const month = val._d.getMonth();
        const day = val._d.getDate();
        history.push(`/app/detail/reserve?conferenceRoomId=${conferenceRoomId}&month=${month}&day=${day}`);
    }
        ...
        return(
            <div>
                ...
                <div className='formBody'>
                    <Calendar dateCellRender={this.dateCellRender} monthCellRender={this.monthCellRender} onSelect={this.selectHandle} disabledDate={this.disabledDate}/>
                </div>
            </div>
        )
    }

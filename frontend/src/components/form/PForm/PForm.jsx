...
import PCollectionCreateForm from './PCustomizedForm';
import PFormTable from './PFormTable';
...
export default class PForm extends Component{
    ...
    onDelete = (positionId) => {
        ...
        this.setState({ dataSource: dataSource.filter(item => item.positionId !== positionId) });

        console.log(positionId);
        //Delete方法
        var apiUrl = '/sm/position/position/' + positionId;
    ...
    //点击修改
    editClick = (positionId) => {
        const form = this.form;
        const { dataSource } = this.state;
        const index = catchIndex(dataSource, positionId);
        console.log(positionId);
        form.setFieldsValue({
            positionId: dataSource[index].positionId,
            positionName: dataSource[index].positionName,
            positionCreatorName: dataSource[index].positionCreatorName,
            // address: dataSource[index].address.split(' / '),
            positionCreatedTime: dataSource[index].positionCreatedTime,
            positionUpdatorName: dataSource[index].positionUpdatorName,
            positionUpdatedTime: dataSource[index].positionUpdatedTime,
        });
        this.setState({
            visible: true,
            tableRowKey: positionId,
            isUpdate: true,
        });
    };
    //更新修改
    ...
    render(){
        const { employeeName, address, timeRange, dataSource, visible, isUpdate, loading } = this.state;
        const questiontxt = ()=>{
            return (
                <p>
                ...
                <div className='formBody'>
                    <Row gutter={16}>
                        <Col className="gutter-row" sm={8}>
                            <Search
                                placeholder="Input PositionName"
                    ...
                    />
                    {isUpdate?
                        <PCollectionCreateForm ref={this.saveFormRef} visible={visible} onCancel={this.handleCancel} onCreate={this.handleUpdate} title="Update Info" okText="update"
                        /> : <PCollectionCreateForm ref={this.saveFormRef} visible={visible} onCancel={this.handleCancel} onCreate={this.handleCreate} title="Create Info" okText="create"
                        />}
                </div>
            </div>
        )
